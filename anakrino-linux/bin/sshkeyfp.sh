...
function do_fingerprint()
{
    ...
    then
        ...
        then
            sha256=$(ssh-keygen -E sha256 -l -f "$file" |awk '{print $2}' |sed -e 's/SHA256://')
        elif [[ -f "${file}.pub" ]]
        then
            ...
            then
                sha256=$(awk '{print $2}' "${file}.pub" |base64 -d |sha256sum -b |awk '{print $1}'|xxd -r -p |base64 |sed -e 's/=\+$//')
            else
                sha256=''
            fi
        else
...
do
    ...
    then
        ...
        do
            ...
            then
                md5=$(echo "${aka[1]}" |base64 -d|md5sum |awk '{print $1}'|sed -e 's/\(..\)/:\1/g' |sed -e 's/^://')
            fi
            if which base64 sha256sum >/dev/null 2>&1
            then
                sha256=$(echo "${aka[1]}" |base64 -d|sha256sum |awk '{print $1}')
            fi
            keyalgo=${aka[0]}
            keyowner=${aka[2]}
            fileowner=$(stat -c "%U" "$file")
