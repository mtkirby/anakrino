...
for o, a in opts:
    if o in ("--file"):
        ...
        splunkUrl += '/app/search/search?earliest=-7d%40h&lastest=now&q=search%20index%3Danakrino%20sourcetype%3Dsudoers'
    elif o in ("--mailfrom"):
        mailFrom = a
    elif o in ("--mailto"):
        mailTo = a
    ...
    with gzip.open(splunkReportFile, 'rt') as f:
        ...
        for row in splunkReader:
            try:
                ...
                report += '<td>' + row['user_alias'] + '</td>'
                report += '<td>' + row['hosts'] + '</td>'
                report += '<td>' + row['hosts_alias'] + '</td>'
                report += '<td>' + row['runas'] + '</td>'
                report += '<td>' + row['runas_alias'] + '</td>'
                report += '<td>' + row['auth'] + '</td>'
                report += '<td>' + row['cmnd'] + '</td>'
                report += '<td>' + row['cmnd_alias'] + '</td>'
                report += '<td>' + row['ALERT'] + '</td>'
                report += '</tr>' + "\n"
            except:
                pass
        f.close()
