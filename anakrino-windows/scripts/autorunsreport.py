...
for o, a in opts:
    if o in ("--file"):
        ...
        splunkUrl += '/app/search/search?earliest=-7d%40h&lastest=now&q=search%20index%3Dautoruns'
    elif o in ("--mailfrom"):
        mailFrom = a
    elif o in ("--mailto"):
        mailTo = a
    ...
    with gzip.open(splunkReportFile, 'rt') as f:
        ...
        for row in splunkReader:
            try:
                ...
                report += '<td>' + row['FileTimeStamp'] + '</td>'
                report += '<td>' + row['EntryLocation'] + '</td>'
                report += '<td>' + row['Description'] + '</td>'
                report += '<td>' + row['Publisher'] + '</td>'
                report += '<td>' + row['ImagePath'] + '</td>'
                report += '</tr>' + "\n"
            except:
                pass
        f.close()
